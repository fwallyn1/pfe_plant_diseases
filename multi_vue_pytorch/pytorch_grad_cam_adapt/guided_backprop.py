...
from pytorch_grad_cam_adapt.utils import replace_all_layer_type_recursive
...
class GuidedBackpropReLUModel:
    ...
    def __call__(self, input_imgs, target_category=None):
        replace_all_layer_type_recursive(self.model,
                                         torch.nn.ReLU,
                                         GuidedBackpropReLUasModule())

        if self.cuda:
            for input_img in input_imgs :input_img = input_img.cuda()

        for input_img in input_imgs : input_img.requires_grad_(True)
        for params in self.model.parameters():
            params.requires_grad = True
        output = self.forward(input_imgs)
        ...
        for input_img in input_imgs:
            ...
            outputs.append(copy(output))
        replace_all_layer_type_recursive(self.model,
                                         GuidedBackpropReLUasModule,
                                         torch.nn.ReLU())
        for input_img in input_imgs : input_img.requires_grad_(False)
        for params in self.model.parameters():
