...
def tensor2cam(image, cam1,cam2):
...
def image2cam(image, cam1):
    image_np = image.squeeze().permute(1,2,0).cpu().numpy()
    
    cam1 = np.uint8(cam1 * 255.0)
    img_with_cam = cv2.applyColorMap(cam1, cv2.COLORMAP_JET)
    img_with_cam = cv2.cvtColor(img_with_cam, cv2.COLOR_BGR2RGB)
    img_with_cam = img_with_cam + (image_np * 255)
...
def subplot(images, nrows, ncols, parse=lambda x: x, rows_titles=None, cols_titles=None, title=''):
    ...
    try:
        for row, subfig in enumerate(subfigs):
            subfig.suptitle(rows_titles[row])
            try:
                axs = subfig.subplots(nrows=1, ncols=ncols)
                print(axs)
                for col, ax in enumerate(axs):
                    if cols_titles is not None:  ax.set_title(cols_titles[col])
...
def vis_outs2images_classes(outs):
    imagesRecto = [x[0][0] for x in outs]
    imagesVerso = [x[0][1] for x in outs]
    classes = [x[1]['prediction'][0] for x in outs]
    return imagesRecto, imagesVerso,classes

"""def vis_outs2images_classes(outs):
    imagesRecto = [x[0][0] for x in outs]
    #imagesVerso = [x[0][1] for x in outs]
    classes = [x[1]['prediction'][0] for x in outs]
    return imagesRecto,classes"""