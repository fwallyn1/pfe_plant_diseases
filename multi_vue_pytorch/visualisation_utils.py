...
def tensor2cam(image, cam1,cam2):
    image_with_heatmap = image2cam(image.squeeze().permute(1,2,0).cpu().numpy(),
              cam1.detach().cpu().numpy(),cam2.detach().cpu().numpy())

    return torch.from_numpy(image_with_heatmap).permute(2,0,1)

def image2cam(image, cam1,cam2):
    h, w, c = image.shape
    cam1 -= min(np.min(cam1),np.min(cam2))
    cam1 /= max(np.max(cam1),np.max(cam2))  # Normalize between 0-1
    cam1 = cv2.resize(cam1, (w,h))
